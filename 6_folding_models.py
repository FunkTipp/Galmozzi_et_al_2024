...
def res_saturation(contact_map, last_emerged):
	...
	#2: all stable contacts for DnaK model
	#3: all interdomain contacts
	#4: additional interdomain conatcs for DnaK model
...
for p, file in enumerate(os.listdir(path + '')):
	if file.endswith(".cif"):
		...
		# <= 6 angstroms distance
		# >= 6 residues distance on the peptide chain
		# am pLDDT >= 70
		contacts_p = get_contacts(coord_array, 6, 6, pLDDT, 70)
...
for g_count, (gene, contacts_length) in enumerate(contact_maps_out.items()):
	...
	#row 0 = TF binding model
 	#row 1 = DnaK binding model
	chaperone_binding_outfile = np.zeros((gene_length, 2))
	...
	for ts in np.arange(gene_length):
		...
		if compact_ids_C == 0:
			continue
		chaperone_binding_outfile[ts, 0] = np.sum(residue_mat[:compact_ids_C, 4])
		...
		if K2_compact_ids_C == 0:
			continue
		chaperone_binding_outfile[ts, 1] = np.sum(residue_mat[:K2_compact_ids_C, 2])+np.sum(residue_mat[:K2_compact_ids_C, 3])
