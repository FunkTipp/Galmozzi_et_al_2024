...
for p, file in enumerate(os.listdir(path + 'path_to_alphfold_mmcif_files/')):
	if file.endswith(".cif"):
		try:
			urllib.request.urlretrieve("https://alphafold.ebi.ac.uk/files/"+file[:-13]+"-predicted_aligned_error_v2.json", \
					path + '/_temp.json')
			A = open(path + '/_temp.json')
			B = json.load(A)[0]
			C = np.zeros((max(B['residue1']), max(B['residue1'])), dtype=np.int16)
			C[np.array(B['residue1'])-1, np.array(B['residue2'])-1] = np.array(B['distance'])
			with open(path + "outpath_to_PAE_files/PAE_"+file[:-13]+".pick", "wb") as handle:
...
with open(path + "/Users/.../cath_coli.pick", "rb") as handle:
...
for p, file in enumerate(os.listdir(path + 'path_to_alphfold_mmcif_files/')):
	if file.endswith(".cif"):
		#read the mmcif file
		inflie = open(path + 'path_to_alphfold_mmcif_files/' + file).read()
		#get gene name
		...
		try:
			with open(path + "outpath_to_PAE_files/PAE_"+file[:-13]+".pick", "rb") as handle:
...
#save contact maps
with open(path + "contact_maps.pick", "wb") as handle:
...
for g_count, (gene, contacts_length) in enumerate(contact_maps_out.items()):
	...
	for ts in np.arange(gene_length):
		...
		if last_emerged < 0:
			continue
		#get emerged residues
		emerged_res = np.arange(last_emerged)
		...
		####TF model####
		TF_P = res_mat[:, 4]*res_mat[:, 4].reshape(last_emerged, -1)
		chaperone_binding_outfile[ts, 0] = np.sum(TF_P[start:end, start:end])
		####DnaK model 1####
		K_P = res_mat[:, 2]*res_mat[:, 2].reshape(last_emerged, -1)
		chaperone_binding_outfile[ts, 1] = np.sum(K_P[start:end, start:end])
		####DnaK model 2####
		K2_stable_res_A = residue_mat[:, 1]
		K2_stable_res_B = np.amin([np.cumsum(K2_stable_res_A), np.cumsum(sK2_table_res_A[::-1])[::-1]], axis=0)
		K2_stable_ids = np.where(K2_stable_res_B > 10)[0]
		try:
			K2_start = np.min(K2_stable_ids)
			K2_end = np.max(K2_stable_ids)
		except:
			K2_start = last_emerged
			K2_end = last_emerged
		K2_P = (res_mat[:, 2]*res_mat[:, 2].reshape(last_emerged, -1)) + \
					(res_mat[:, 3]*res_mat[:, 3].reshape(last_emerged, -1))
		chaperone_binding_outfile[ts, 2] = np.sum(K2_P[K2_start:K2_end, K2_start:K2_end])

	master[gene] = chaperone_binding_outfile
with open("/Users/.../chaperone_binding_out.pick", "wb") as handle:
