...
for g_count, (gene, contacts_length) in enumerate(contact_maps_out.items()):
	...
	for ts in np.arange(gene_length):
		...
		####TF model####
		TF_P = residue_mat[:, 4]*residue_mat[:, 4].reshape(last_emerged, -1)
		chaperone_binding_outfile[ts, 0] = np.sum(TF_P[start:end, start:end])
		####DnaK model 1####
		K_P = residue_mat[:, 2]*residue_mat[:, 2].reshape(last_emerged, -1)
		chaperone_binding_outfile[ts, 1] = np.sum(K_P[start:end, start:end])
		####DnaK model 2####
		K2_stable_res_A = residue_mat[:, 1]
		K2_stable_res_B = np.amin([np.cumsum(K2_stable_res_A), np.cumsum(K2_stable_res_A[::-1])[::-1]], axis=0)
		...
		try:
			...
			K2_end = last_emerged
		K2_P = (residue_mat[:, 2]*residue_mat[:, 2].reshape(last_emerged, -1)) + \
					(residue_mat[:, 3]*residue_mat[:, 3].reshape(last_emerged, -1))
