...
#load amino acid sequence file
with open(path + "aa_seq_MC4100.pick", "rb") as handle: #path to aa_seq files
...
for g_count, (gene, contacts_length) in enumerate(contact_maps_out.items()):
	...
	#row 0 = TF molten globule binding model
	chaperone_binding_outfile = np.zeros((gene_length, 0))
	...
	for ts in np.arange(gene_length):
		...
		##molten globule
		if np.sum(np.sum(residue_mat[:, 4])) != 0:
			chaperone_binding_outfile[ts, 0] = np.sum(residue_mat[:, 4]) + np.sum(residue_mat[:, 0]) - old_stab
		else:
			old_stab = np.sum(residue_mat[:, 0])
